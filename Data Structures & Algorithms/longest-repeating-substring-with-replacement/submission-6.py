...
- We compare the max value to the current window length, and choose the maximum from the two values on each iteration to be stored in max value. This is due to the window
size constantly changing size, so we want to keep record the max length recorded every iteration.
