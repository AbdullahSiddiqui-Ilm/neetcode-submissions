...
class Solution:    
    def mergeKLists(self, lists: List[Optional[ListNode]]) -> Optional[ListNode]:
        ...
        while len(lists) > 1:
            i = 1
            while i < len(lists):
                lists[i - 1] = (self.mergeTwoLists(lists[i - 1], lists[i]))
                lists.pop(i)  # this is not ideal as it shifts every element after once to the left, multiple times inside the loop
                i += 2
        return lists[0]

        """ merged = []      #This version is O(n) space due to merged []
        while len(lists) > 1:
            i = 1
            if len(lists) % 2 != 0:
